...
from app.schemas.book import (
    ...
    BookCreate, BookUpdate, GenreResponse
)
...
async def search_books(
    ...
    genre_id: Optional[int] = Query(None, description="Filter by genre ID"),
    min_rating: Optional[float] = Query(None, ge=0.0, le=5.0, description="Minimum average rating"),
    published_year_start: Optional[int] = Query(None, ge=1000, description="Earliest publication year"),
    published_year_end: Optional[int] = Query(None, le=2030, description="Latest publication year"),
    page: int = Query(1, ge=1, description="Page number"),
    per_page: int = Query(10, ge=1, le=100, description="Items per page"),
    db: Session = Depends(get_db),
    current_user: Optional[User] = Depends(get_optional_current_user)
