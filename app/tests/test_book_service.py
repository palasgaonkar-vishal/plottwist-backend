...
# pytest used for fixtures, patch and MagicMock for potential future mocking
# patch and MagicMock available for future mocking needs
...
class TestBookService:
    ...
    def test_get_books_pagination(
        self, db_session: Session, multiple_books: list[Book]
    ):
    ...
    def test_get_books_with_genre_filter(
        self, db_session: Session, multiple_books: list[Book], sample_genre: Genre
    ):
    ...
    def test_get_books_with_rating_filter(
        self, db_session: Session, multiple_books: list[Book]
    ):
    ...
    def test_search_books_by_title(
        ...
        assert len(books) >= 1
        assert any(
            multiple_books[0].title.lower() in book.title.lower() for book in books
        )
    ...
    def test_search_books_by_author(
        ...
        assert len(books) >= 1
        assert any(
            multiple_books[0].author.lower() in book.author.lower() for book in books
        )

    def test_search_books_with_year_filter(
        self, db_session: Session, multiple_books: list[Book]
    ):
        ...
        search_params = BookSearchQuery(
            published_year_start=2019, published_year_end=2021, page=1, per_page=10
        )
    ...
    def test_update_book_success(
        self, db_session: Session, sample_book: Book, sample_genre: Genre
    ):
    ...
    def test_update_book_statistics(
        self, db_session: Session, sample_book: Book, sample_user: User
    ):
...
class TestGenreService:
    ...
    def test_create_genre_success(self, db_session: Session):
        ...
        genre = service.create_genre(
            name="Test Genre", description="A test genre description"
        )
    ...
    def test_update_genre_success(self, db_session: Session, sample_genre: Genre):
        ...
        updated_genre = service.update_genre(
            sample_genre.id, name="Updated Genre", description="Updated description"
        )
    ...
    def test_update_genre_partial(self, db_session: Session, sample_genre: Genre):
        ...
        updated_genre = service.update_genre(
            sample_genre.id, name="Partially Updated Genre"
        )
