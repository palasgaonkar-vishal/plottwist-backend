...
class BookService:
    ...
    def create_book(self, book_data: BookCreate) -> Book:
        ...
        if book_data.genre_ids:
            genres = self.db.query(Genre).filter(Genre.id.in_(book_data.genre_ids)).all()
            for genre in genres:
    ...
    def update_book_statistics(self, book_id: int) -> Optional[Book]:
        ...
        logger.info(f"Updated statistics for book: {book.title} - Rating: {book.average_rating}, Reviews: {book.total_reviews}")
        return book
...
class GenreService:
    ...
    def update_genre(self, genre_id: int, name: Optional[str] = None, description: Optional[str] = None) -> Optional[Genre]:
        """Update a genre."""
