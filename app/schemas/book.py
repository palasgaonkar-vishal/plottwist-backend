...
class BookCreate(BookBase):
    ...
    genre_ids: List[int] = Field(
        default_factory=list, description="List of genre IDs to associate with the book"
    )
...
class BookUpdate(BaseModel):
    ...
    genre_ids: Optional[List[int]] = Field(
        None, description="List of genre IDs to associate with the book"
    )
...
class BookSearchQuery(BaseModel):
    ...
    published_year_start: Optional[int] = Field(
        None, ge=1000, description="Earliest publication year"
    )
    published_year_end: Optional[int] = Field(
